...
export class CharacterController {
  ...
  public setWalkAnim(
    rangeName: string | AnimationGroup,
    rate: number,
    loop: boolean
  ) {
  ...
  public setTurnRightAnim(
    ...
    this._copySlowAnims(
      this._actionMap.turnRightFast,
      this._actionMap.turnRight
    );
  }
  public setTurnRightFastAnim(
    rangeName: string | AnimationGroup,
    rate: number,
    loop: boolean
  ) {
  ...
  private _setRHS(mesh: TransformNode) {
    ...
    const _localX = Vector3.FromArray(
      <DeepImmutable<Float32Array>>meshMatrix.m,
      0
    );
    const _localY = Vector3.FromArray(
      <DeepImmutable<Float32Array>>meshMatrix.m,
      4
    );
    const _localZ = Vector3.FromArray(
      <DeepImmutable<Float32Array>>meshMatrix.m,
      8
    );
      ...
      if (this._prevActData !== actData) {
        if (actData.exist) {
          ...
          if (this._isAG) {
            ...
            let a: Animatable = this._skeleton.beginAnimation(
              actData.name,
              actData.loop,
              actData.rate
            );
            ...
            c =
              this._skeleton.getAnimationRange(actData.name).to -
              this._skeleton.getAnimationRange(actData.name).from;
          }
    ...
    if (this._hasCam)
      if (this._mode != 1) {
        ...
        if (this._noRot) {
          ...
          if (this._hasCam) this._avatar.rotation.y = ca;
        }
      }
  }
  ...
  private _rotateAVnC(
    ...
    if (
      ...
      if (this._mode == 1) {
        ...
        if (this._hasCam)
          this._camera.alpha =
            this._camera.alpha + this._rhsSign * turnAngle * a;
      }
  ...
  private _updateTargetValue() {
    ...
    if (this._camera.radius <= this._camera.lowerRadiusLimit) {
      if (!this._noFirstPerson && !this._inFP) {
        ...
        this._mode = 0;
        this._inFP = true;
        this._avatar.getChildMeshes().map((m) => {
          if (m.name === `avatarBall`) {
            return;
          }
  ...
  private _skelDrivenByAG(skeleton: Skeleton) {
    return skeleton.animations.some((sa) =>
      this._scene.animationGroups.some((ag) =>
        ag.children.some((ta) => ta.animation == sa)
      )
    );
  }
